...
| [`autoresearch`](autoresearch/SKILL.md) | Autonomous iterative experimentation loop for any programming task with a measurable metric. Guides the user through defining a goal, metric command, scope, and constraints, then runs an autonomous edit → commit → measure → keep-or-revert loop on an isolated branch — advancing only on improvements — and reports the results. Ported from [github/awesome-copilot](https://github.com/github/awesome-copilot/blob/main/skills/autoresearch/SKILL.md) (MIT, by luiscantero), inspired by [Karpathy's autoresearch](https://github.com/karpathy/autoresearch). |
