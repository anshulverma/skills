...
| [`diff-comment-authoring`](diff-comment-authoring/SKILL.md) | How to write a comment or reply on a Phabricator diff: one sentence, leading with the disposition (fixed, declining, fixed in D…), answering only what was asked. Kills the tells that make a reply read as generated — headers and bold labels on a two-line answer, praise openers, restating the reviewer's point, pasted verification output, and volunteered extras like why you got it wrong originally or which precedent you should have used. Applies to replies to automated reviewers too. The reply-side companion to `diff-authoring`, whose titles and summaries *do* take structure. |
